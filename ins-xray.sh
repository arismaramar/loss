...
touch /user/current
clear
echo "IP=$domain" > /var/lib/sc-prem/ipvps.conf
if [[ "$IP" = "" ]]; then
...
# install nginx
mkdir -p /home/vps/public_html
wget -q -O /etc/nginx/conf.d/vps.conf "https://raw.githubusercontent.com/arismaramar/loss/main/badvpn/vps.conf"
sleep 1
wget -q -O xraymode.sh https://raw.githubusercontent.com/arismaramar/loss/main/xraymode.sh && chmod +x xraymode.sh && ./xraymode.sh
sleep 2 
wget -q -O /etc/xray/config.json "https://raw.githubusercontent.com/arismaramar/loss/main/conf/config.json"
chmod +x /etc/xray/config.json
sleep 1 
rm -f /etc/nginx/conf.d/xray.conf
wget -q -O /etc/nginx/conf.d/xray.conf "https://raw.githubusercontent.com/arismaramar/loss/main/conf/xray.conf"
